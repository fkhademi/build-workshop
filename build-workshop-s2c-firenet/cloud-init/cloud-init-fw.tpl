...
sudo mv /etc/nginx/sites-enabled/default /root/
...
sudo sed -i '/ufw-before-forward.*icmp/s/ACCEPT/LOG --log-prefix \"[UFW BLOCK] \"/g' /etc/ufw/before.rules
...
curl -d '{"attributes":{"fieldAttrs":"{}","title":"filebeat-*","timeFieldName":"@timestamp","fields":"[]","typeMeta":"{}","runtimeFieldMap":"{}"}}' -H "Content-Type: application/json" -H "kbn-xsrf: true" -X POST "http://localhost:82/api/saved_objects/index-pattern"

echo "[INFO] $(date) Download Kibana dashboard" >> $LOG
wget https://avx-build.s3.eu-central-1.amazonaws.com/dashboard-aws.json -P /tmp/

sleep 10
echo "[INFO] $(date) Install Kibana Dashboard" >> $LOG
curl -X POST -H "Content-Type: application/json" -H "kbn-xsrf: true" -d @/tmp/dashboard-aws.json "http://localhost:82/api/kibana/dashboards/import" >> $LOG
